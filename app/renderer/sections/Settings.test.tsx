...
describe('Settings', () => {
  beforeEach(() => {
    ...
    mocks.getPlans.mockResolvedValue({ currency: 'EUR', today: { cost: 0, savings: 0, calls: 0 }, month: { cost: 0, savings: 0, calls: 0 }, plans: { claude: { id: 'claude-max', provider: 'claude', budget: 200, spent: 48, percentUsed: 24, projectedMonthEnd: 120, daysUntilReset: 19, periodStart: '2026-07-01', periodEnd: '2026-08-01' } } })
  ...
  it('stores a positive daily token budget from General', async () => {
    const user = userEvent.setup()
    render(<Settings period="month" />)
    await user.click(screen.getByLabelText('Daily token budget'))
    await user.click(screen.getByRole('option', { name: 'Tokens' }))
    await user.type(screen.getByLabelText('Daily token budget amount'), '25000')
    expect(JSON.parse(localStorage.getItem('codeburn.dailyBudget')!)).toEqual({ kind: 'tokens', value: 25000 })
  })
  ...
  it('rejects a non-positive daily budget without persisting it', async () => {
    ...
    await user.click(screen.getByLabelText('Daily token budget'))
    await user.click(screen.getByRole('option', { name: 'Tokens' }))
    await user.type(screen.getByLabelText('Daily token budget amount'), '-5')
  ...
  it('lists, removes, and adds plans through the action bridge', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Plans' }))
    expect((await screen.findAllByText('Claude Max 20x')).length).toBeGreaterThan(0)
    expect(screen.getByText('$200.00/month · claude · 24% API-equivalent value')).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Remove' }))
    await user.click(screen.getByRole('button', { name: 'Confirm' }))
