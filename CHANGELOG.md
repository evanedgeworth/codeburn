...
### Added
- **Cursor server-export reconciliation.** `codeburn cursor-import <usage.csv>` imports Cursor dashboard usage without browser cookies or private APIs, deduplicates overlapping exports, and makes server token and cost totals authoritative for covered periods while preserving local project, session, tool, and command attribution. The macOS billing view separates server-measured Cursor usage from the locally estimated remainder.
